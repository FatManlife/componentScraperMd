...
// Product details
	productColly.OnHTML("div#product-product", func(h *colly.HTMLElement) {
		...
		switch category {
		case "cpu": storage.cpuHandler(h)
		case "gpu": storage.gpuHandler(h)
		case "motherboard": storage.motherboardHandler(h)
		case "ram": storage.ramHandler(h)
		case "hdd": storage.hddHandler(h)
		case "ssd": storage.ssdHandler(h)
		case "psu": storage.psuHandler(h)
		case "case": storage.caseHandler(h)
		case "cooler": storage.coolerHandler(h)
		case "fan": storage.fanHandler(h)
		case "pc": storage.pcHandler(h)
		case "laptop": storage.laptopHandler(h)
		case "aio": storage.aioHandler(h)
		case "mini_pc": storage.pcMiniHandler(h)
		}
	})
}