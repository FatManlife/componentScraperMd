...
func requestBodyProduct(categoryColly *colly.Collector, pageColly *colly.Collector, productColly *colly.Collector, productLinks *chan dto.Link, semaphor *chan struct{}, s *rawsql.Storage) {
	...
	productColly.OnHTML("div.container.page_product",func(h *colly.HTMLElement){
		...
		switch category {
			case "cpu": handler.cpuHandler(h)
			case "motherboard": handler.motherboardHandler(h)
			case "gpu": handler.gpuHandler(h)
			case "ram": handler.ramHandler(h)
			case "ssd": handler.ssdHandler(h)
			case "hdd": handler.hddHandler(h)
			case "fan": handler.fanHandler(h)
			case "case": handler.caseHandler(h)
			case "psu": handler.psuHandler(h)
			case "cooler": handler.coolerHandler(h)
			case "laptop": handler.laptopHandler(h)
			case "pc": handler.pcHandler(h)
			case "aio": handler.aioHandler(h)
			case "mini_pc": handler.pcMiniHandler(h)
		}
	})	
}
